function Search-Registry { 
                    ...
                    ForEach-Object { 
                        ...
                        if ($ValueNameRegex) {  
                            ...
                            if ($Key.GetValueNames() -match $ValueNameRegex) {  
                                ...
                                return [PSCustomObject] @{ 
                                    Key = $Key 
                                    Match = $key.GetValueNames() | Where-Object {$_ -match $ValueNameRegex}
                                    Reason = "ValueName" 
                                } 
                            }  
                        } 
                        ...
                        if ($ValueDataRegex) {  
                            ...
                            if (($Key.GetValueNames() | foreach-object { $Key.GetValue($_) }) -match $ValueDataRegex) {  
                                ...
                                return [PSCustomObject] @{ 
                                    Key = $Key 
                                    Match = ($Key.GetValueNames() | foreach-object { $Key.GetValue($_) }) | where-object {$_ -match $ValueDataRegex}
                                    Reason = "ValueData" 
                                } 
                            } 
                        } 
    ...
    Search-Registry -Path "HKLM:\SYSTEM"  -Recurse -SearchRegex "arctic" -ea SilentlyContinue | format-list | Out-File C:\temp\regmatch.txt -force
    Search-Registry -Path "HKLM:\SOFTWARE"  -Recurse -SearchRegex "arctic" -ea SilentlyContinue | Format-List | Out-File C:\temp\regmatch.txt -Append